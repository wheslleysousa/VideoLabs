...
app=FastAPI(title=APP_NAME,version='0.3.0');app.add_middleware(CORSMiddleware,allow_origins=['https://videolabs.vercel.app','http://localhost:5173'],allow_origin_regex=r'https://.*\.vercel\.app',allow_credentials=False,allow_methods=['*'],allow_headers=['*'])
...
def public_job(i):
 with lock:
  ...
  return {'id':i,'status':j['status'],'phase':j.get('phase',''),'progress':j.get('progress',0),'total':j.get('total',0),'percent':round(j.get('progress',0)/max(j.get('total',1),1)*100,1),'error':j.get('error'),'created_at':j['created_at'],'expires_at':j['created_at']+JOB_TTL_SECONDS,'download_url':f'/v1/jobs/{i}/download' if j['status']=='ready' else None}
...
def normalize(src,dst):
 ...
 if has_audio(src):cmd+=['-vf',vf,'-c:v','libx264','-preset','veryfast','-crf','24','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-ar','48000','-ac','2','-movflags','+faststart',str(dst)]
 else:cmd+=['-f','lavfi','-i','anullsrc=channel_layout=stereo:sample_rate=48000','-map','0:v:0','-map','1:a:0','-shortest','-vf',vf,'-c:v','libx264','-preset','veryfast','-crf','24','-pix_fmt','yuv420p','-c:a','aac','-b:a','128k','-ar','48000','-ac','2','-movflags','+faststart',str(dst)]
...
def download(url,target):
 req=urllib.request.Request(url,headers={'User-Agent':'VideoLabs/0.3'});total=0
 with urllib.request.urlopen(req,timeout=120) as r,target.open('wb') as f:
  while True:
   b=r.read(1024*1024)
   if not b:break
   total+=len(b)
   if total>105*1024*1024:raise RuntimeError('Um vídeo remoto excede o limite de 100 MB.')
   f.write(b)
def process_job(i):
 with lock:j=jobs[i];wd=Path(j['workdir']);stages=j['stages'];mode=j['mode'];requested=j['requested']
 try:
  inputs=wd/'inputs';norm=wd/'normalized';inputs.mkdir(exist_ok=True);norm.mkdir(exist_ok=True);local=[];n=sum(len(s['files']) for s in stages);done=0
  for si,s in enumerate(stages):
   arr=[]
   for fi,f in enumerate(s['files']):
    set_job(i,status='processing',phase=f'Baixando vídeo {done+1}/{n}')
    ext=Path(f.get('name','video.mp4')).suffix or '.mp4';src=inputs/f's{si:02d}_f{fi:03d}{ext}';download(f['url'],src);dst=norm/f's{si:02d}_f{fi:03d}.mp4';set_job(i,phase=f'Normalizando vídeo {done+1}/{n}');normalize(src,dst);arr.append(dst);src.unlink(missing_ok=True);done+=1
   local.append(arr)
  ...
  set_job(i,total=len(selected),progress=0,phase='Gerando combinações');zp=wd/'VideoLabs-resultados.zip';manifest={'project':j['project_name'],'mode':mode,'total_available':len(combos),'generated':len(selected),'stages':[{'name':s['name'],'files':[f['name'] for f in s['files']]} for s in stages],'outputs':[]}
  with zipfile.ZipFile(zp,'w',compression=zipfile.ZIP_STORED,allowZip64=True) as z:
   for x,c in enumerate(selected,1):
    out=wd/f'VideoLabs-{x:04d}.mp4';concat(c,out);z.write(out,arcname=out.name);manifest['outputs'].append({'file':out.name,'sources':[p.name for p in c]});out.unlink(missing_ok=True);set_job(i,progress=x,phase=f'Gerando {x}/{len(selected)}')
   z.writestr('manifest.json',json.dumps(manifest,ensure_ascii=False,indent=2))
  shutil.rmtree(inputs,ignore_errors=True);shutil.rmtree(norm,ignore_errors=True);set_job(i,status='ready',phase='Pronto para baixar',progress=len(selected),zip_path=str(zp))
...
@app.get('/')
def root():return {'name':APP_NAME,'status':'ok','version':'0.3.0'}
@app.get('/health')
def health():return {'ok':True,'ffmpeg':Path(FFMPEG).name,'version':'0.3.0'}
...
async def create_remote(request:Request):
 ...
 for s in stages:
  ...
  for f in s['files']:
   u=str(f.get('url',''))
   if not u.startswith('https://res.cloudinary.com/sskapqzv/'):raise HTTPException(400,'Origem de vídeo não permitida.')
 total=1
 ...
 with lock:jobs[i]={'status':'queued','phase':'Na fila','progress':0,'total':total if mode=='all' else min(requested,total),'error':None,'created_at':time.time(),'workdir':str(wd),'stages':stages,'mode':mode,'requested':requested,'project_name':safe_name(m.get('projectName','Projeto VideoLabs'))}
 executor.submit(process_job,i);return public_job(i)
