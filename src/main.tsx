...
type Job={id:string;status:string;phase:string;progress:number;total:number;percent:number;quality?:string;error?:string|null;download_url?:string|null};
...
function App(){
 ...
 const change=async(id:string,list:FileList|null)=>{
  ...
  if(!stage)return;
  const base=stage.files.length;
  const prefix=stagePrefix(stage.name);
  const incoming=Array.from(list).map((file,index)=>({id:uid(),file,label:`${prefix}${base+index+1}`,status:'uploading' as const,progress:0}));
 ...
 const generate=async()=>{
  ...
  try{
   const uploaded=stages.map(s=>({name:s.name,files:s.files.map(c=>({...c.remote!,label:c.label.trim()||stagePrefix(s.name)}))}));
   const r=await fetch(`${API}/v1/jobs/remote`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectName,mode,requested:randomN,quality,stages:uploaded})});
   let data:any={};
 ...
 return <main>
  ...
  <section className="hero"><div className="pill"><Sparkles size={14}/> COMBINAÇÕES AUTOMÁTICAS</div><h1>Um upload.<br/><em>Centenas de vídeos.</em></h1><p>Adicione suas etapas, nomeie os clipes e gere combinações prontas para baixar.</p></section>
  <section className="workspace">
   <div className="projectBar">
    <label><small>NOME DO PROJETO</small><input value={projectName} onChange={e=>setProjectName(e.target.value)}/></label>
   ...
   <div className="qualityBar">
    <div><small>QUALIDADE DE SAÍDA</small><strong>{quality==='high'?'1080p — melhor qualidade':'720p — processamento mais rápido'}</strong></div>
    <div className="qualityOptions">
     <button className={quality==='high'?'active':''} onClick={()=>setQuality('high')}><Clapperboard size={15}/><span><b>1080p</b><small>Alta</small></span></button>
     <button className={quality==='fast'?'active':''} onClick={()=>setQuality('fast')}><Gauge size={15}/><span><b>720p</b><small>Rápida</small></span></button>
    </div>
   </div>
   ...
   {stages.length===0?<div className="emptyState"><div><Plus size={22}/></div><strong>Nenhuma etapa ainda</strong><p>Comece adicionando a primeira etapa. Ex.: Hook, Corpo, CTA.</p><button className="add" onClick={add}><Plus size={17}/> Adicionar primeira etapa</button></div>:
   <div className="stages">{stages.map((s,i)=><article className="stage" key={s.id}>
    <div className="stageTop"><b>{String(i+1).padStart(2,'0')}</b><input value={s.name} onChange={e=>setStages(a=>a.map(x=>x.id===s.id?{...x,name:e.target.value}:x))}/><button onClick={()=>setStages(a=>a.filter(x=>x.id!==s.id))}><Trash2 size={16}/></button></div>
    ...
    {s.files.length>0&&<div className="files">{s.files.map(c=><div className="fileRow" key={c.id}>
     <input className="clipName" value={c.label} maxLength={32} onChange={e=>patchClip(s.id,c.id,{label:e.target.value})} aria-label={`Nome de ${c.file.name}`}/>
     <div className="fileMeta"><span title={c.file.name}>{c.file.name}</span><em>{c.status==='uploaded'?'enviado':c.status==='error'?'erro':`${c.progress}%`}</em></div>
     {c.status==='error'&&<button title="Tentar novamente" onClick={()=>void retryClip(s.id,c.id)}><RotateCcw size={13}/></button>}
     <button title="Remover" onClick={()=>removeFile(s.id,c.id)}>×</button>
    </div>)}</div>}
   </article>)}</div>}

   {job?<div className={`job ${job.status}`}><div className="jobTop"><div><small>PROCESSAMENTO</small><strong>{job.phase||job.status}</strong></div><b>{job.percent||0}%</b></div><div className="progress"><i style={{width:`${job.percent||0}%`}}/></div><p>{job.status==='error'?(job.error||'Falha no processamento.'):job.status==='ready'?`${job.total} vídeo(s) finalizado(s) em ${job.quality==='fast'?'720p rápido':'1080p alta'}.`:`${job.progress||0} de ${job.total||0} combinações concluídas.`}</p>{job.status==='ready'&&<a className="download" href={`${API}${job.download_url}`}><Download size={17}/> Baixar ZIP</a>}</div>:
